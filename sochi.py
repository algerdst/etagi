...
headers = {
  ...
  'Cookie': '_ga_sync=wrokGWQkGI+QJABTCsPRAg=='
}
...
with open('sochi.csv', 'w', newline='', encoding='utf-8-sig') as file:
    ...
    for url in urls:
        ...
        while True:
            if page == 0:
                ...
                if index==0:
                    link=url[:930]+str(page)+url[931:]
                elif index==1:
                    link=url[:893]+str(page)+url[894:]
                elif index==2:
                    link=url[:960]+str(page)+url[961:]
                elif index==3:
                    link=url[:788]+str(page)+url[789:]
...
print(f'Сбор данных занял {finish-start} cекунд')
